...
export class KnowledgePanelComponent extends LitElement {
  static override styles = css`
    ...
    .table_element th, .table_element td {
      border: 1px solid #ddd;
      padding: 0.625rem;
      text-align: left;
    }
   ...
   */
  private _knowledgePanelsTask = new Task(
    this,
    {
        ...
        return html`
          <div class="table_element">
            ...
            <table>
              <thead>
                <tr>
                  ${columns.map((column: TableColumn) => html`<th>${column.text || column.id || ''}</th>`)}
                </tr>
              </thead>
              <tbody>
                ${rows.map((row: TableRow) => html`
                  <tr>
                    ${(row.cells || []).map((cell: TableCell) => html`<td>${cell.text || cell.value || ''}</td>`)}
                  </tr>
                `)}
              </tbody>
            </table>
        ...
        if (element.panel_element?.panel_id) {
          ...
          return html`
            <div class="sub-panel">
              ...
              <div class="elements">
                ${element.elements.map((subElement: KnowledgePanelElement) => this.renderElement(subElement))}
              </div>
            </div>
          `;
        }
      ...
      case 'panel_group': {
        ...
        return html`
          <div class="panel-group">
            ...
            ${panelGroup.image ? html`
              <div class="panel-image">
                <img 
                  src="${panelGroup.image.sizes?.['400']?.url || panelGroup.image.sizes?.['full']?.url || ''}" 
      ...
      <div class="panel ${panel.level || ''} ${panel.size || ''}">
        ${title ? html`
          <div class="panel-header">
            ...
            ${panel.title_element?.subtitle ? html`<div class="panel-subtitle">${panel.title_element.subtitle}</div>` : ''}
          </div>
        ` : ''}
  ...
  override render(): TemplateResult {
    return this._knowledgePanelsTask.render({
      ...
      complete: (result) => {
        ...
        const topLevelPanels = topLevelPanelIds
          .filter(id => panels[id])
          .map(id => panels[id]);
        
        // If no top-level panels found, show all panels
        const panelsToRender = topLevelPanels.length > 0 ? 
          topLevelPanels : 
      ...
      error: (error: unknown) => {
        ...
        return html`<div class="error">${error instanceof Error ? error.message : String(error)}</div>`;
      },
    });
  }
}
