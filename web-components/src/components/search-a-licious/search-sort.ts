...
import { SortParameters, SearchaliciousSortInterface } from "./interfaces/sort-interfaces"
  ...
  implements SearchaliciousSortInterface
{
   ...
   */
  currentSortOption() {
    return (this.sortOptions().filter((node) => node.selected) ?? [undefined])[0]
  }
  ...
  setSortOptionById(optionId: string | undefined) {
    this.sortOptions().forEach(
      (node) => ((node as SearchaliciousSortOption).selected = node.id === optionId)
    )
  }
  ...
  override disconnectedCallback() {
    ...
    this.removeEventHandler(SearchaliciousEvents.SORT_OPTION_SELECTED, (event) =>
      this._handleSelected(event)
    )
  }
}
