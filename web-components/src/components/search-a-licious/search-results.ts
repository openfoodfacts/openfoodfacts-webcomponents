...
import { MissingResultTemplateError, MultipleResultTemplateError } from "./errors"
...
@localized()
export class SearchaliciousResults extends SignalWatcher(SearchaliciousResultCtlMixin(LitElement)) {
  ...
  _getTemplate(name: string) {
    ...
    if (!slots || slots.length === 0) {
      ...
      throw new MultipleResultTemplateError(`Multiple slots found with name="${name}"`)
    }
  ...
  renderResults() {
    ...
    const renderResult = (result: Object, index: number) => this.resultRenderer(html, result, index)
    ...
    return html` <ul part="results">
      ${repeat(this.searchResultDetail.results, KeyFnOrTemplate, templateOrUndef)}
    </ul>`
  }
