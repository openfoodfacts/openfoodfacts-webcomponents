...
export class SearchaliciousFacet extends LitElement {
  ...
  reset = (submit?: boolean): void => {
    throw new Error(`reset not implemented: implement in sub class with submit ${submit}`)
  }
  setSelectedTerms(terms: string[]) {
    throw new Error(`setSelectedTerms not implemented: implement in sub class ${terms.join(", ")}`)
  }
...
export class SearchaliciousTermsFacet extends SearchActionMixin(
  SearchaliciousTermsMixin(SearchCtlGetMixin(DebounceMixin(SearchaliciousFacet)))
) {
  ...
  override searchFilter(): string | undefined {
    ...
    values = values.map((value) => (!value.match(/^\w+$/) ? `"${value}"` : value))
    if (values.length === 0) {
      return undefined
    }
  ...
  renderAddTerm() {
    ...
    const otherItem = this.infos!.items?.find((item) => item.key === FACET_TERM_OTHER) as
      | FacetTerm
      | undefined
    ...
    return html`
      <div class="add-term" part="add-term">
        <label for="${inputName}"
          >${getPluralTranslation(otherItem?.count, msg("Other"), msg("Others"))}
      ...
      <div>
        ...
        >
          ...
          <div slot="label" style="display: contents;">
            ${term.name}
            ${term.count ? html`<span part="docCount">(${term.count})</span>` : nothing}
          </div>
        </searchalicious-checkbox>
      </div>
    `
