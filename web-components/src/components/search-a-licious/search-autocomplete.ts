...
import { SuggestionSelectionMixin, SuggestionSelectionResult } from "./mixins/suggestion-selection"
...
@localized()
export class SearchaliciousAutocomplete extends SuggestionSelectionMixin(LitElement) {
  static override styles = css`
    ...
    ul li:hover,
    ul li.selected {
      background-color: var(--searchalicious-autocomplete-selected-background-color, #cfac9e);
    }
  ...
  override handleInput(value: string) {
    ...
    const inputEvent = new CustomEvent(SearchaliciousEvents.AUTOCOMPLETE_INPUT, {
      detail: { value: value },
      bubbles: true,
      composed: true,
    })
  ...
  override submitSuggestion(isSuggestion = false) {
    ...
    const inputEvent = new CustomEvent(SearchaliciousEvents.AUTOCOMPLETE_SUBMIT, {
  ...
  override render() {
    return html`
      <span class="search-autocomplete" part="search-autocomplete">
        ...
        >
          ${this.isLoading ? html`<li>${msg("Loading...")}</li>` : this._renderSuggestions()}
        </ul>
      </span>
    `
  }
