...
import { SuggestionSelectionOption, SuggestOption } from "./interfaces/suggestion-interfaces"
...
export class SearchaliciousTaxonomySuggester extends SearchaliciousTermsMixin(
  ...
  selectTermByTaxonomy(taxonomy: string, term: string) {
    for (const facets of this.searchCtl!.relatedFacets()) {
      ...
      if ((facets as SearchaliciousFacetsInterface).selectTermByTaxonomy(taxonomy, term)) {
        return
      }
    }
  ...
  override async selectSuggestion(selected: SuggestionSelectionOption) {
    this.selectTermByTaxonomy((selected as taxonomySelectionOption).taxonomy, selected.value)
  }

  override renderSuggestion(suggestion: SuggestOption, _index: number): TemplateResult {
