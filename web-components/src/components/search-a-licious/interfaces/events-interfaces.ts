export interface EventRegistrationInterface {
   ...
   */
  addEventHandler(event: string, handler: EventListenerOrEventListenerObject): void
  /**
   * Removes window.removeEventListener but only if really needed
   * @param event - event name
   * @param handler - function handling event.
   */
  removeEventHandler(event: string, handler: EventListenerOrEventListenerObject): void
}
