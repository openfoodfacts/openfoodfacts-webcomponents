...
export class SearchaliciousToggle extends CheckedInputMixin(LitElement) {
  ...
  static override styles = css`
    ...
    .slider {
      ...
      right: 0;
      bottom: 0;
      background-color: var(--searchalicious-toggle-inactive-background-color, grey);
      transition: 0.4s;
      border-radius: 17px;
    }
    ...
    input[type="checkbox"]:checked + .slider {
      background-color: var(--searchalicious-toggle-active-background-color, black);
    }
  ...
  override render() {
    return html`
      <div class="toggle-wrapper">
        <label for="${this.name}"><slot name="label">${this.label}</slot></label>
        <div class="toggle">
          <input
            part="toggle-input"
            .name=${this.name}
