...
export class SearchaliciousPages extends SearchaliciousResultCtlMixin(LitElement) {
  ...
  _computeRange(currentPage?: number, pageCount?: number) {
    ...
    let start = this.displayedPages > 2 ? Math.max(currentPage - 1, 1) : currentPage
  ...
  get _isFirstPage() {
    return Boolean(this.searchResultDetail.currentPage && this.searchResultDetail.currentPage === 1)
  }
  get _isLastPage() {
    return Boolean(
        ...
        <ul part="pages">
          ${this.displayFirst
            ? html` <li @click=${this._firstPage} part="first">
                <slot name="first"><button ?disabled=${this._isFirstPage}>|&lt;</button></slot>
              </li>`
            : ""}
          <li @click=${this._prevPage} part="previous">
            <slot name="previous"><button ?disabled=${this._isFirstPage}>&lt;</button></slot>
          </li>
          ...
          ${repeat(
            ...
            (page, _) =>
              html` <li
                class="${page === this.searchResultDetail.currentPage! ? "current" : ""}"
                part="${page === this.searchResultDetail.currentPage! ? "current-" : ""}page"
              >
          ...
          <li @click=${this._nextPage} part="next">
            <slot name="next"><button ?disabled=${this._isLastPage}>&gt;</button></slot>
          </li>
          ${this.displayLast
            ? html` <li @click=${this._lastPage} part="last">
                <slot name="last"><button ?disabled=${this._isLastPage}>&gt;|</button></slot>
              </li>`
            : nothing}
        </ul>
      </nav>
  ...
  _nextPage() {
    this._askPageChange(
      Math.min(this.searchResultDetail.currentPage! + 1, this.searchResultDetail.pageCount!)
    )
  }
