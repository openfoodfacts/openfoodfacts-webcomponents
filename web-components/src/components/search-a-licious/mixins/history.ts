...
import { addParamPrefixes, removeParamPrefixes, removeParenthesis } from "../utils/url"
...
> = {
  ...
  [HistorySearchParams.FACETS_FILTERS]: (history) => {
    ...
    const selectedTermsByFacet = history.facetsFilters.split(QueryOperator.AND).reduce(
      (acc, filter) => {
 ...
 * @constructor
 */
export const SearchaliciousHistoryMixin = <T extends Constructor<LitElement>>(superClass: T) => {
  class SearchaliciousHistoryMixinClass extends superClass {
    ...
    convertHistoryParamsToValues = (params: URLSearchParams): HistoryOutput => {
      ...
      const history = removeParamPrefixes(Object.fromEntries(params), this.name)
  ...
  return SearchaliciousHistoryMixinClass as Constructor<SearchaliciousHistoryInterface> & T
}
