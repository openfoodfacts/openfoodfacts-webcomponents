 ...
 */
export interface SearchaliciousTaxonomiesInterface extends VersioningMixinInterface {
  terms: TermOption[]
   ...
   */
  getTaxonomiesTerms(q: string, taxonomyNames: string[]): Promise<TaxomiesTermsResponse>
}
...
export const SearchaliciousTermsMixin = <T extends Constructor<LitElement>>(
  ...
  class SearchaliciousTermsMixinClass extends VersioningMixin(superClass) {
    ...
    termLabel(term: TermOption): string {
      return term.text + (term.name && term.name != term.text ? ` (${term.name})` : "")
    }
  ...
  return SearchaliciousTermsMixinClass as Constructor<SearchaliciousTaxonomiesInterface> & T
}
