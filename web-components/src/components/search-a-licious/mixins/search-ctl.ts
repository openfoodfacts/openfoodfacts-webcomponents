...
import { API_LIST_DIVIDER, DEFAULT_SEARCH_NAME, PROPERTY_LIST_DIVIDER } from "../utils/constants"
...
import { canResetSearch, isSearchChanged, isSearchLoading, searchResultDetail } from "../signals"
import { SignalWatcher } from "@lit-labs/preact-signals"
import { isTheSameSearchName } from "../utils/search"

export const SearchaliciousSearchMixin = <T extends Constructor<LitElement>>(superClass: T) => {
  /**
  ...
  class SearchaliciousSearchMixinClass extends SignalWatcher(
    ...
    _chartParams(isGetRequest: boolean): ChartSearchParam[] | string | undefined {
      ...
      document
        .querySelectorAll(`searchalicious-distribution-chart[search-name=${this.name}]`)
        .forEach((item) => {
      ...
      document
        .querySelectorAll(`searchalicious-scatter-chart[search-name=${this.name}]`)
        .forEach((item) => {
    ...
    override connectedCallback() {
      ...
      this.addEventHandler(SearchaliciousEvents.FACET_SELECTED, (event: Event) => {
    ...
    override disconnectedCallback() {
      ...
      this.removeEventHandler(SearchaliciousEvents.LAUNCH_SEARCH, (event: Event) =>
        this._handleSearch(event)
      )
      this.removeEventHandler(SearchaliciousEvents.CHANGE_PAGE, (event) =>
        this._handleChangePage(event)
      )
      this.removeEventHandler(SearchaliciousEvents.LAUNCH_FIRST_SEARCH, (event) =>
        this.search((event as CustomEvent)?.detail[HistorySearchParams.PAGE])
      )
    ...
    buildParams = (page?: number) => {
      ...
      const params: SearchParameters = {
        ...
        langs: this.langs.split(PROPERTY_LIST_DIVIDER).map((lang) => lang.trim()),
    ...
    async search(page = 1) {
      ...
      let response
      if (method === "GET") {
        response = await fetch(`${searchUrl}?${this._paramsToQueryStr(params)}`)
      } else {
  ...
  return SearchaliciousSearchMixinClass as Constructor<SearchaliciousSearchInterface> & T
}
